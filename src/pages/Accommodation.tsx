                    ...
                    {rooms.map(room => (
                      <TableRow key={room.id} className="border-b last:border-b-0 hover:bg-slate-50/30">
                        ...
                        {daysOfWeek.map(day => {
                          ...
                          // Find all bookings overlapping this date for this room
                          ...
                          const activeBooking = matchingBookings.find(b => b.status !== 'CHECKED OUT' && b.status !== 'VACANT');
                          const checkedOutBooking = matchingBookings.find(b => b.status === 'CHECKED OUT');

                          const displayBooking = activeBooking || checkedOutBooking;

                          if (displayBooking) {
                            const isCheckedIn = displayBooking.status === 'CHECKED IN';
                            const isCheckedOut = displayBooking.status === 'CHECKED OUT';
                            return (
                              <TableCell
                                key={day.toISOString()}
                                onClick={() => {
                                  if (isCheckedOut) {
                                    // If it's checked out and there's no active booking, allow adding a new booking
                                    handleOpenAddBooking(String(room.id), dateStr);
                                  } else {
                                    handleOpenEditBooking(displayBooking);
                                  }
                                }}
                                className={cn(
                                  "p-2 text-center border-r align-middle cursor-pointer transition-all hover:brightness-95 relative group",
                              ...
                              >
                                <div className={cn(
                                  ...
                                  <div className="flex items-center gap-1.5 w-full justify-center">
                                    <span className="font-bold text-[11px] truncate max-w-[80px]">{displayBooking.customerName.split(' ')[0]}</span>
                                  </div>
                                  <span className="text-[9px] font-bold opacity-75 uppercase tracking-wider">{displayBooking.status}</span>
                                </div>
                                {isCheckedOut && (
                                  <div className="absolute inset-0 bg-black/5 dark:bg-white/5 opacity-0 group-hover:opacity-100 flex items-center justify-center rounded-xl transition-opacity">
                                    <span className="bg-primary text-white text-[10px] font-bold px-2 py-1 rounded-lg shadow-sm">Allocate New</span>
                                  </div>
                                )}
                              </TableCell>
                            );
            ...
            <Tabs value={bookingActiveTab} onValueChange={setBookingActiveTab} className="w-full">
              <TabsList className="bg-slate-55 dark:bg-slate-800 p-0.5 rounded-lg flex overflow-x-auto scrollbar-none w-full max-w-full sm:w-fit gap-1 border">
                ...
                <TabsTrigger value="schedules" className="rounded-md px-4 py-1.5 text-xs font-semibold shrink-0">
                  Nightly Schedules <span className="ml-1 bg-slate-200 dark:bg-slate-750 px-1.5 py-0.5 rounded-full text-[10px]">{bookingSummary.nights}</span>
                </TabsTrigger>
              </TabsList>
            </Tabs>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3.5 text-xs">
                  ...
                  <div className="flex justify-between items-center text-slate-550">
                    ...
                    <span className="font-semibold text-slate-800 dark:text-slate-200">KES {(bookingSummary.totalRoomRate + bookingSummary.packageRate * bookingSummary.nights).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                  </div>

...
const newSummary = `const bookingSummary = useMemo(() => {
      ...
      if (bookingForm.roomAllocations && bookingForm.roomAllocations.length > 0) {
         totalRoomRate = bookingForm.roomAllocations.reduce((sum, alloc) => sum + alloc.nightlyRate, 0);
...
// Update the UI to use totalRoomRate instead of nightlyTotal * nights
content = content.replace(
  /\{\(bookingSummary\.nightlyTotal \* bookingSummary\.nights\)\.toLocaleString\(undefined, \{ minimumFractionDigits: 2 \}\)\}/,
  "{ (bookingSummary.totalRoomRate + bookingSummary.packageRate * bookingSummary.nights).toLocaleString(undefined, { minimumFractionDigits: 2 }) }"
);

fs.writeFileSync(path, content);
console.log('Patched bookingSummary for dynamic room allocation rates');
